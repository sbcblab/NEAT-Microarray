    ...
    with open(save_dir + 'db_stats.txt', 'w') as db_stats:
        ...
        if te_inputs is not None:
            ...
            for gene in xrange(tr_inputs.shape[1]):
                ...
                genes_pvalues.append(stats.kruskal(tr_gene_expression, te_gene_expression)[1])
            ...
            for gtr, gte in zip(genes_tr, genes_te):
                genes_C00.append(stats.kruskal(gtr[0], gte[0])[1])
                genes_C11.append(stats.kruskal(gtr[1], gte[1])[1])
                genes_C01.append(stats.kruskal(gtr[0], gte[1])[1])
                genes_C10.append(stats.kruskal(gtr[1], gte[0])[1])
                  
                genes_TR.append(stats.kruskal(gtr[0], gtr[1])[1])   
                genes_TE.append(stats.kruskal(gte[0], gte[1])[1])  
...
def run(nothing, tr_inputs, tr_outputs, te_inputs=None, te_outputs=None, tr_names=None, te_names=None, features_names=None, classes_labels=None, save_dir='', num_cores=1, num_gen=1):
    ...
    if te_inputs is not None and te_outputs is not None:
        ...
        for en in error_names:
            ERROR_NAMES.append(en)    
    return k_conf_matrix, acc, ERROR_NAMES
...
if __name__ == '__main__':
    ...
    if independent_test:
        ...
        ML_frame.start(None, run, training, testing=testing, k=k, save_dir=save_dir, num_cores=num_cores, num_gen=num_gen)
        ...
        ML_frame.start(None, run, training, k=k, save_dir=save_dir, num_cores=num_cores, num_gen=num_gen)
        visualize.plot_genes_stats(GENES, k, filename=save_dir+'genes')
        visualize.plot_genes_selection(NUM_GENES, training.number_data_features, filename=save_dir+'genes_selected') 
