...
def save_selection_history(history, best, pv, probs, features_names, filename):
    best_filename = filename.replace('.csv', 'best.csv')
    
    sor = zip(features_names, pv, probs)
    sor.sort(key=lambda tup: tup[2], reverse=True)
    
    with open(filename, 'w') as history_file:
        first_line = 'GENE, PVAL, PROB'
        for g in xrange(len(history)):
            first_line = first_line + ', G{}'.format(g)
        first_line = first_line + '\n'
        history_file.write(first_line)    
        for gene, pv, p in sor:
            index = -(features_names.index(gene)+1)
            new_line = '{}, {}, {}'.format(gene, pv, p)
            for generation in history:
                if index in generation:
                    count = generation[index]
                else:
                    count = 0    
                new_line = new_line + ', {:0>4}'.format(count)
            new_line = new_line + '\n'
            history_file.write(new_line)
            
    with open(best_filename, 'w') as best_file:
        first_line = 'GENE, PVAL, PROB'
        for g in xrange(len(best)):
            first_line = first_line + ', G{}'.format(g)
        first_line = first_line + '\n'
        best_file.write(first_line)    
        for gene, pv, p in sor:
            index = -(features_names.index(gene)+1)
            new_line = '{}, {}, {}'.format(gene, pv, p)
            for generation in best:
                if index in generation:
                    count = 1
                else:
                    count = 0    
                new_line = new_line + ', {}'.format(count)
            new_line = new_line + '\n'
            best_file.write(new_line)                 
