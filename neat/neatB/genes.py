...
from neatB.attributes import FloatAttribute, BoolAttribute, StringAttribute
...
class BaseGene(object):
    ...
    def mutate(self, config, mutate_power_decay):
        for a in self._gene_attributes:
            v = getattr(self, a.name)
            setattr(self, a.name, a.mutate_value(v, config, mutate_power_decay))
    ...
    def crossover(self, gene2):
        ...
        new_gene = self.__class__(self.key)
        for a in self._gene_attributes:
            #if type(a) == FloatAttribute and random() < 0.6:
            #    setattr(new_gene, a.name, (getattr(self, a.name) + getattr(self, a.name) / 2.0))
            #else:
            if random() > 0.5:
                setattr(new_gene, a.name, getattr(self, a.name))
            else:
