...
from neatB.activations import ActivationFunctionSet
from neat.aggregations import AggregationFunctionSet
from neatB.config import ConfigParameter, write_pretty_params
from neatB.genes import DefaultConnectionGene, DefaultNodeGene
...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, params, num_generations, probs):
...
class DefaultGenome(object):
    ...
    @classmethod
    def parse_config(cls, param_dict, num_generations, probs):
        param_dict['node_gene_type'] = DefaultNodeGene
        param_dict['connection_gene_type'] = DefaultConnectionGene
        return DefaultGenomeConfig(param_dict, num_generations, probs)
    ...
    def __init__(self, key):
        ...
        # Fitness results.
        self.fitness = None
        self.parent1_fit = None
        self.parent2_fit = None
        self.parent1_key = None
        self.parent2_key = None
    ...
    def configure_new(self, config):
        ...
        for node_key in config.output_keys:
            #CHANGE CONFIG VARIABLE HERE TO USE MGAUSS IN OUTPUTS!
            #output_config = copy.deepcopy(config)
            #self.nodes[node_key] = self.create_node(output_config, node_key)
    ...
    def configure_crossover(self, genome1, genome2, config):
        ...
        self.parent1_fit = parent1.fitness
        self.parent2_fit = parent2.fitness
        self.parent1_key = parent1.key
        self.parent2_key = parent2.key

        self.generation = parent1.generation + 1.0
        ...
        for key, cg2 in iteritems(parent2.connections):
            ...
            if cg1 is None and cg2.enabled and key[0] < 0 and key[1] in parent1.nodes:
                if random() < 0.5:
    ...
    '''def configure_crossover(self, genome1, genome2, config):
        ...
        for key, cg1 in iteritems(parent1.connections):
            ...
            if cg2 is None:
                # Excess or disjoint gene: copy from the fittest parent.
                self.connections[key] = cg1.copy()
                if cg1.enabled == False:
                    self.connections[key].enabled = False
            else:
                # Homologous gene: combine genes from both parents.
                self.connections[key] = cg1.crossover(cg2)
                if cg1.enabled == False or cg2.enabled == False:
                    self.connections[key].enabled = False
    ...
    def mutate(self, config):
        ...
        #cap = 1.0 - (self.generation / (config.num_gen ))
        #cdp = self.generation / (config.num_gen * 2.0)       
        cap = config.conn_add_prob
        cdp = config.conn_delete_prob
        ...
        if config.single_structural_mutation:
            ...
            if random() < cap:
                self.mutate_swap_input(config)                
                
            if random() < cap:
                self.mutate_add_input(config)        
            
    def mutate_not_structural(self, config):
        # Mutate connection genes.
        
        #mutate_power_decay = np.power(0.99, self.generation-1)
        mutate_power_decay = 1.0
        
        for cg in self.connections.values():
            cg.mutate(config, mutate_power_decay)

        # Mutate node genes (bias, response, etc.).
        for ng in self.nodes.values():
            ng.mutate(config, mutate_power_decay)                    
    ...
    def mutate_add_input(self, config):
        possible_outputs = config.output_keys
        possible_inputs = config.input_keys        
        redo = True
        i = 0
        while redo:
            in_node = np.random.choice(possible_inputs, p=config.prob)
            ...
            for out_node in possible_outputs:
                ...
                if key in self.connections:
                    if config.check_structural_mutation_surer():
                        if self.connections[key].enabled == False:
    ...
    def mutate_swap_input(self, config):
        if self.connections:
            possible_inputs = config.input_keys        
            redo = True
            i = 0
            while redo:
                ...
                new_input = np.random.choice(possible_inputs)
                if key[0] < 0 and key[0] != new_input and self.connections[key].enabled == True:
                    swap = self.create_connection(config, new_input, key[1])
                    swap.weight = self.connections[key].weight
                    del self.connections[key]
                    self.connections[swap.key] = swap   
                    redo = False     
                if i > 20:
                    redo = False
                i += 1
    ...
    def mutate_add_connection(self, config):
        ...
        for key in self.connections:
            if key[0] < 0:
                selected_inputs.append(key[0])
        possible_inputs = [node for node in possible_outputs + selected_inputs if node not in config.output_keys]
    ...
    '''def mutate_add_connection(self, config):
        ...
        possible_gene = config.input_keys
        
        #possible_inputs = possible_outputs + config.input_keys
        in_node1 = np.random.choice(possible_gene, p=config.prob)
        #in_node1 = choice(possible_gene)
        try:
            in_node2 = choice([node for node in possible_outputs if node not in config.output_keys])
        except:
            in_node2 = None
        if random() < 0.5 or len(possible_outputs) <= len(config.output_keys) or in_node2 is None:
            in_node = in_node1
        else:
            in_node = in_node2
        ...
        for out_node in possible_outputs:
            ...
            if key in self.connections:
                # TODO: Should this be using mutation to/from rates? Hairy to configure...
                if config.check_structural_mutation_surer():
                    if not self.connections[key].enabled:
                        self.connections[key].enabled = True
                    else:    
                        return
                #skip = True
    ...
    '''def fix_topology(self, config):
        used_nodes = set(required_for_output(config.input_keys, config.output_keys, self.connections.keys()))     
    ...
    '''def mutate_delete_connection(self, config):
        if self.connections:
            possible_gene = config.input_keys
            #possible_inputs = possible_outputs + config.input_keys
            del_key = np.random.choice(possible_gene)
    ...
    def __str__(self):
        s = "Key: {0}\nFitness: {1}\nP1 Fit: {2} ({3})\nP2 Fit: {4} ({5})\nNodes:".format(self.key, self.fitness, self.parent1_fit, self.parent1_key, self.parent2_fit, self.parent2_key)
    ...
    def create_node(config, node_id):
        ...
        #CHANGE HERE TO DEFINE ACTIVATION FOR OUTPUT NODE
        if node_id == 0:
            node.activation = 'mgauss'
        return node
    ...
    def connect_partial_nodirect(self, config):
        ...
        with (unless no hidden nodes) no direct input-output connections."""
        assert 0 <= config.connection_fraction <= 1
        all_inputs  = config.input_keys 
        all_outputs = config.output_keys
        #shuffle(all_inputs)
        num_to_add = int(round(len(all_inputs) * config.connection_fraction))
        selected = np.random.choice(all_inputs, num_to_add, replace=False, p=config.prob)
